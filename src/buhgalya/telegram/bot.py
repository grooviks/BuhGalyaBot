...
async def api_request(
    ...
    async with httpx.AsyncClient(base_url=str(get_settings().api_base_url), timeout=10) as client:
...
async def debt_add(message: Message, command: CommandObject) -> None:
    ...
    await message.answer(
        f"Долг добавлен: {debt['person_name']} — {debt['balance_rub']} ₽ (ID: {debt['id']})"
    )
...
async def debt_repay(message: Message, command: CommandObject) -> None:
    ...
    try:
        ...
        return
    await message.answer(
        f"Возврат записан. {debt['person_name']}: остаток {debt['balance_rub']} ₽."
    )
...
async def debts(message: Message) -> None:
    ...
    lines.extend(
        f"• {debt['person_name']}: {debt['balance_rub']} ₽ (ID: {debt['id']})" for debt in active
    )
    await message.answer("\n".join(lines))
